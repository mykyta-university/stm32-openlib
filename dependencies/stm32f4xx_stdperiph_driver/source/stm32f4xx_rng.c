 ...
 *** Interrupts ***
 ...
 [..] If enabled, an RNG interrupt is pending :
   ...
   (#) In the case of the RNG_DR register contains valid random data. 
       This interrupt library is cleared once the RNG_DR register has been read
   ...
   (#) In the case of a seed error : One of the following faulty sequences has 
       ...
       This interrupt library is cleared using RNG_ClearITPendingBit(RNG_IT_SEI)
       function; or
   (#) In the case of a clock error : the PLL48CLK (RNG peripheral clock library)
       was not correctly detected (fPLL48CLK< fHCLK/16). This interrupt library is
 ...
 *** Managing the RNG controller events : ***
 ...
 [..] The user should identify which mode will be used in his application to manage 
   ...
   (#)  In the Interrupt Mode it is advised to use the following functions:
        (++) RNG_ITConfig()       : to enable or disable the interrupt library.
...
/**
  ...
  * @param  RNG_IT: specifies the RNG interrupt library to check.
  *          This parameter can be one of the following values:
