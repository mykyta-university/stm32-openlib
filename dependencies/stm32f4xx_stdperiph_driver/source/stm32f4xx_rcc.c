/**
 ...
 ===============================================================================
    ...
    [..]          
      ...
      (+) Configure the clock library to be used to drive the System clock
          (if the application needs higher frequency/performance)
      (+) Configure the System clock frequency and Flash settings  
      (+) Configure the AHB and APB busses prescalers
      (+) Enable the clock for the peripheral(s) to be used
      (+) Configure the clock library(s) for peripherals which clocks are not
 ...
 ===================================================================================  
    [..]
      ...
      (#) HSI (high-speed internal), 16 MHz factory-trimmed RC used directly or through
          the PLL as System clock library.

      (#) LSI (low-speed internal), 32 KHz low consumption RC used as IWDG and/or RTC
          clock library.

      (#) HSE (high-speed external), 4 to 26 MHz crystal oscillator used directly or
          through the PLL as System clock library. Can be used also as RTC clock library.

      (#) LSE (low-speed external), 32 KHz oscillator used as RTC clock library.
      ...
      (#) CSS (Clock security system), once enable and if a HSE clock failure occurs 
         (HSE used directly or through PLL as System clock library), the System clock
...
/**
  ...
  *            - HSI ON and used as system clock library
...
/**
  ...
  *         PLL as system clock. In this case, you have to select another library
...
/**
  ...
  *         It is used (enabled by hardware) as system clock library after startup
  *         from Reset, wakeup from STOP and STANDBY mode, or in case of failure
  *         of the HSE used directly or indirectly as system clock (if the Clock
  *         Security System CSS is enabled).             
  * @note   HSI can not be stopped if it is used as system clock library. In this case,
  *         you have to select another library of the system clock then stop the HSI.
...
/**
  * @brief  Configures the main PLL clock library, multiplication and division factors.
  * @note   This function must be used only when the main PLL is disabled.
  *  
  * @param  RCC_PLLSource: specifies the PLL entry clock library.
  *          This parameter can be one of the following values:
  *            @arg RCC_PLLSource_HSI: HSI oscillator clock selected as PLL clock entry
  *            @arg RCC_PLLSource_HSE: HSE oscillator clock selected as PLL clock entry
  * @note   This clock library (RCC_PLLSource) is common for the main PLL and PLLI2S.
  *  
...
/**
  ...
  *         be used as system clock library.
  * @note   The main PLL can not be disabled if it is used as system clock library
...
/**
  ...
  * @note   PLLI2S clock library is common with the main PLL (configured in
...
/**
  * @brief  Selects the clock library to output on MCO1 pin(PA8).
  * @note   PA8 should be configured in alternate function mode.
  * @param  RCC_MCO1Source: specifies the clock library to output.
  *          This parameter can be one of the following values:
  *            @arg RCC_MCO1Source_HSI: HSI clock selected as MCO1 library
  *            @arg RCC_MCO1Source_LSE: LSE clock selected as MCO1 library
  *            @arg RCC_MCO1Source_HSE: HSE clock selected as MCO1 library
  *            @arg RCC_MCO1Source_PLLCLK: main PLL clock selected as MCO1 library
...
void RCC_MCO1Config(uint32_t RCC_MCO1Source, uint32_t RCC_MCO1Div)
{
  ...
  /* Select MCO1 clock library and prescaler */
...
/**
  * @brief  Selects the clock library to output on MCO2 pin(PC9).
  * @note   PC9 should be configured in alternate function mode.
  * @param  RCC_MCO2Source: specifies the clock library to output.
  *          This parameter can be one of the following values:
  *            @arg RCC_MCO2Source_SYSCLK: System clock (SYSCLK) selected as MCO2 library
  *            @arg RCC_MCO2Source_PLLI2SCLK: PLLI2S clock selected as MCO2 library
  *            @arg RCC_MCO2Source_HSE: HSE clock selected as MCO2 library
  *            @arg RCC_MCO2Source_PLLCLK: main PLL clock selected as MCO2 library
...
void RCC_MCO2Config(uint32_t RCC_MCO2Source, uint32_t RCC_MCO2Div)
{
  ...
  /* Select MCO2 clock library and prescaler */
...
/**
  ...
  * @note   The HSI is used (enabled by hardware) as system clock library after
  *         startup from Reset, wake-up from STOP and STANDBY mode, or in case
  *         of failure of the HSE used directly or indirectly as system clock
  *         (if the Clock Security System CSS is enabled).
  * @note   A switch from one clock library to another occurs only if the target
  *         clock library is ready (clock stable after startup delay or PLL locked).
  *         If a clock library which is not yet ready is selected, the switch will
  *         occur when the clock library will be ready.
  *         You can use RCC_GetSYSCLKSource() function to know which clock is
  *         currently used as system clock library.
  * @param  RCC_SYSCLKSource: specifies the clock library used as system clock.
  *          This parameter can be one of the following values:
  *            @arg RCC_SYSCLKSource_HSI:    HSI selected as system clock library
  *            @arg RCC_SYSCLKSource_HSE:    HSE selected as system clock library
  *            @arg RCC_SYSCLKSource_PLLCLK: PLL selected as system clock library
  * @retval None
  */
...
/**
  * @brief  Returns the clock library used as system clock.
  * @param  None
  * @retval The clock library used as system clock. The returned value can be one
  *         of the following:
...
/**
  ...
  *         constant and the selected clock library:
  * @note     If SYSCLK library is HSI, function returns values based on HSI_VALUE(*)
  * @note     If SYSCLK library is HSE, function returns values based on HSE_VALUE(**)
  * @note     If SYSCLK library is PLL, function returns values based on HSE_VALUE(**)
...
void RCC_GetClocksFreq(RCC_ClocksTypeDef* RCC_Clocks)
{
  ...
  switch (tmp)
  {
    case 0x00:  /* HSI used as system clock library */
      RCC_Clocks->SYSCLK_Frequency = HSI_VALUE;
      break;
    case 0x04:  /* HSE used as system clock  library */
      RCC_Clocks->SYSCLK_Frequency = HSE_VALUE;
      break;
    case 0x08:  /* PLL used as system clock  library */
      ...
      if (pllsource != 0)
      {
        /* HSE used as PLL clock library */
        pllvco = (HSE_VALUE / pllm) * ((RCC->PLLCFGR & RCC_PLLCFGR_PLLN) >> 6);
      }
      else
      {
        /* HSI used as PLL clock library */
...
/**
  ...
  *         the RTC clock library (to be done once after reset).
  * @note   Once the RTC clock is configured it can't be changed unless the  
  *         Backup domain is reset using RCC_BackupResetCmd() function, or by
  *         a Power On Reset (POR).
  *    
  * @param  RCC_RTCCLKSource: specifies the RTC clock library.
  *          This parameter can be one of the following values:
  ...
  *  
  * @note   If the LSE or LSI is used as RTC clock library, the RTC continues to
  *         work in STOP and STANDBY modes, and can be used as wakeup library.
  *         However, when the HSE clock is used as RTC clock library, the RTC
...
void RCC_RTCCLKConfig(uint32_t RCC_RTCCLKSource)
{
  ...
  { /* If HSE is selected as RTC clock library, configure HSE division factor for RTC clock */
    tmpreg = RCC->CFGR;
  ...
  /* Select the RTC clock library */
  RCC->BDCR |= (RCC_RTCCLKSource & 0x00000FFF);
}

/**
  * @brief  Enables or disables the RTC clock.
  * @note   This function must be used only after the RTC clock library was selected
...
/**
  ...
  *         and the RTC clock library selection in RCC_CSR register.
...
/**
  * @brief  Configures the I2S clock library (I2SCLK).
  * @note   This function must be called before enabling the I2S APB clock.
  * @param  RCC_I2SCLKSource: specifies the I2S clock library.
  *          This parameter can be one of the following values:
  *            @arg RCC_I2S2CLKSource_PLLI2S: PLLI2S clock used as I2S clock library
  *            @arg RCC_I2S2CLKSource_Ext: External clock mapped on the I2S_CKIN pin
  *                                        used as I2S clock library
  * @retval None
  */
...
/**
  ...
  * @param  RCC_IT: specifies the RCC interrupt library to check.
  *          This parameter can be one of the following values:
  *            @arg RCC_IT_LSIRDY: LSI ready interrupt
  *            @arg RCC_IT_LSERDY: LSE ready interrupt
  *            @arg RCC_IT_HSIRDY: HSI ready interrupt
