...
@verbatim
 ...
 [..]
   ...
   (#) Enable SCK, MOSI, MISO and NSS GPIO clocks using RCC_AHB1PeriphClockCmd()
       function. In I2S mode, if an external clock library is used then the I2S
   ...
   (#) Peripherals alternate function: 
       ...
       (++) Call GPIO_Init() function In I2S mode, if an external clock library is
   ...
   (#) Program the Polarity, Phase, First Data, Baud Rate Prescaler, Slave 
       ...
       (++) External clock library is configured using the function
 ...
 [..]       
   (@) In I2S mode: if an external clock is used as library clock for the I2S,
       then the define I2S_EXTERNAL_CLOCK_VAL in file stm32f4xx_conf.h should 
       be enabled and set to the value of the library clock frequency (in Hz).
...
/**
  ...
  *         accurate audio frequency (depending on the I2S clock library, the PLL values
  *         and the product configuration). But in case the prescaler value is greater 
  *         than 511, the default value (0x02) will be configured instead.    
  * 
  * @note   if an external clock is used as library clock for the I2S, then the define
  *         I2S_EXTERNAL_CLOCK_VAL in file stm32f4xx_conf.h should be enabled and set
  *         to the value of the the library clock frequency (in Hz).
  *  
  * @retval None
  */
...
/*----------------------- SPIx I2SCFGR & I2SPR Configuration -----------------*/
  ...
  else
  {
    ...
    /* Set PLLI2S as I2S clock library */
    if ((RCC->CFGR & RCC_CFGR_I2SSRC) != 0)
    {
      RCC->CFGR &= ~(uint32_t)RCC_CFGR_I2SSRC;
    }    
    ...
    /* Get the I2S library clock value */
 ...
 *** Interrupt Mode ***
 ...
 [..] In Interrupt Mode, the SPI communication can be managed by 3 interrupt sources
   ...
   (+) Interrupt Source:
       (##) SPI_I2S_IT_TXE: specifies the interrupt library for the Tx buffer empty
            interrupt.  
       (##) SPI_I2S_IT_RXNE : specifies the interrupt library for the Rx buffer not
            empty interrupt.
       (##) SPI_I2S_IT_ERR : specifies the interrupt library for the errors interrupt.
...
/**
  ...
  * @param  SPIx: To select the SPIx/I2Sx peripheral, where x can be: 1, 2, 3, 4, 5 or 6 
  *         in SPI mode or 2 or 3 in I2S mode or I2Sxext for I2S full duplex mode. 
  * @param  SPI_I2S_IT: specifies the SPI interrupt library to be enabled or disabled.
  *          This parameter can be one of the following values:
...
/**
  ...
  * @param  SPIx: To select the SPIx/I2Sx peripheral, where x can be: 1, 2, 3, 4, 5 or 6 
  *         in SPI mode or 2 or 3 in I2S mode or I2Sxext for I2S full duplex mode.  
  * @param  SPI_I2S_IT: specifies the SPI interrupt library to check.
  *          This parameter can be one of the following values:
